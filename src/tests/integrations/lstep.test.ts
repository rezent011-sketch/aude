...
import * as lstep from '../../integrations/lstep';

describe('lstep', () => {
  it('parses webhook payload', () => {
    expect(lstep.parseLstepWebhook({ event: 'follow', userId: 'u1', timestamp: 1 })).toEqual(
      expect.objectContaining({ event: 'follow', userId: 'u1', timestamp: 1 })
    );
  });

  it('throws on invalid webhook payload', () => {
    expect(() => lstep.parseLstepWebhook({ event: 'follow' })).toThrow(IntegrationError);
  });

  it('forwards event to discord', async () => {
    ...
    await lstep.forwardToDiscord(client, '123', { event: 'message', userId: 'u1', message: 'hello', timestamp: 1 });
